...
ifdef  WINDOWS
    define Rdest,   rcx, text                    ; dest
    define Rcount,  rdx, text                    ; count
    define Rdest2,  r9, text                     ; copy of dest
    define Rcount2, r8, text                     ; copy of count
else   ; Unix
    define Rdest,   rdi, text                    ; dest
    define Rcount,  rdx, text                    ; count
    define Rdest2,  rcx, text                    ; copy of dest
    define Rcount2, rsi, text                    ; copy of count
endif
...
; a reasonable value is half the size of the largest cache
    MemsetCacheLimit label ptr size_t
        memsetcachelimit_ isize_t 0
...
; memsetCPUDispatch is only executed once:
    memsetDispatch label size_t
        isize_t memsetCPUDispatch

.const

; Jump table for count from 0 to 16:
    MemsetJTab label size_t
        isize_t M00, M01, M02, M03, M04, M05, M06, M07
        isize_t M08, M09, M10, M11, M12, M13, M14, M15, M16
...
_uasm_CPUFeatures_IntrinSet proto callconv (dword)

; Imported from cachesize64.asm:
;extern DataCacheSize                   ; Get size of data cache
_uasm_CPUFeatures_DataCacheSize proto callconv (size_t) :dword
...
_uasm_CPUFeatures_Store256BitIsFaster proto callconv (dword)
...
procstart _uasm_memset, callconv, ptr, < >, < >, _Dest:ptr, _Val:dword, _Size:size_t
A_memset:
?OVR_memset:
...
memsetCPUDispatch label size_t    ; CPU dispatcher, check for instruction sets and which method is fastest        
        ...
        lea     rbx, [memsetSSE2@]
        call    _uasm_CPUFeatures_Store256BitIsFaster    ; Test if 256-bit read/write is available and faster than 128-bit read/write
        test    eax, eax
        jz      Q100
        lea     rbx, [memsetAVX@]
...
; extern "C" size_t GetMemsetCacheLimit(); // Data blocks bigger than this will be stored uncached by memset
procstart _uasm_GetMemsetCacheLimit, callconv, size_t, < >, < >, < >
GetMemsetCacheLimit:
GetMemsetCacheLimit@:
...
%endif
        call    _uasm_CPUFeatures_DataCacheSize          ; get cache size
...
; extern "C" void   SetMemsetCacheLimit(); // Change limit in GetMemsetCacheLimit
procstart _uasm_SetMemsetCacheLimit, callconv, void, < >, < >, < >
SetMemsetCacheLimit:
%ifdef  WINDOWS
        mov     rax, rcx
%else
